...
export async function captureHealthScoreSnapshot(talentId, score, issueData = {}) {
  ...
  const snapshot = await prisma.healthScoreSnapshot.create({
    data: {
      ...
      score,
      previousScore,
      factors: issueData.factors || {},
      triggers,
      metadata: {
        issueCount: issueData.issues?.length || 0,
        criticalIssues: issueData.issues?.filter(i => i.severity === 'critical').length || 0,
        highIssues: issueData.issues?.filter(i => i.severity === 'high').length || 0,
      }
    }
  });
      ...
      await prisma.creatorTask.update({
        ...
        data: {
          ...
          metadata: {
            ...autoTask.metadata,
            autoResolved: true,
            resolvedReason: `Score improved to ${newScore}%`
          }
        }
      });
